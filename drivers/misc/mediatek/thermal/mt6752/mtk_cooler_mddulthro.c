...
#define mtk_cooler_mddulthro_dprintk_always(fmt, args...) \
	do { pr_debug("thermal/cooler/mddulthro" fmt, ##args); } while (0)
...
static struct thermal_cooling_device *cl_mddulthro_dev[MAX_NUM_INSTANCE_MTK_COOLER_MDDULTHRO] = {
	0
};
...
static int mtk_cooler_mddulthro_register_ltf(void)
{
	...
	for (i = MAX_NUM_INSTANCE_MTK_COOLER_MDDULTHRO; i-- > 0;) {
		...
		cl_mddulthro_dev[i] =
		    mtk_thermal_cooling_device_register(temp, (void *)&cl_mddulthro_state[i],
