...
#define mtk_cooler_mddtxpwr_dprintk_always(fmt, args...) \
	do { pr_debug("thermal/cooler/mddtxpwr" fmt, ##args); } while (0)
...
static void mtk_cl_mddtxpwr_set_mddtxpwr_limit(void)
{
	...
	if (min_limit != cl_mddtxpwr_cur_limit) {
		...
		if (65535 <= cl_mddtxpwr_cur_limit) {
			...
			int ret = eemcs_notify_md_by_sys_msg(MD_SYS5, EXT_MD_DTX_REQ,
...
static int mtk_cooler_mddtxpwr_register_ltf(void)
{
	...
	for (i = MAX_NUM_INSTANCE_MTK_COOLER_MDDTXPWR; i-- > 0;) {
		...
		cl_mddtxpwr_dev[i] =
		    mtk_thermal_cooling_device_register(temp, (void *)&cl_mddtxpwr_state[i],
							&mtk_cl_mddtxpwr_ops);
	}
