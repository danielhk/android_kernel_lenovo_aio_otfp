...
static bool mtk_uart_pm_log_en = 0;
/*---------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------*/
struct mtuart_entry pm_log_en_entry = {
	{.name = "pm_log_en", .mode = S_IRUGO | S_IWUSR},
	mtk_uart_pm_log_en_show,
	mtk_uart_pm_log_en_store,
};
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_pm_log_en_show(struct kobject *kobj, char *buffer)
{
	ssize_t len = 0;
	int pm_log_en;

	pm_log_en = mtk_uart_pm_log_en;
	len += scnprintf(buffer + len, PAGE_SIZE - len, "%d\n", pm_log_en);

	return len;
}

/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_pm_log_en_store(struct kobject *kobj, const char *buffer, size_t size)
{
	int a;
	int res = sscanf(buffer, "%d", &a);

	mtk_uart_pm_log_en = a;
