/* mediatek/platform/mt6752/kernel/drivers/uart/uart.c
 *
 * (C) Copyright 2008
...
#ifdef ENABLE_DEBUG
	if ((lsr & UART_LSR_BI) || (lsr & UART_LSR_PE) || (lsr & UART_LSR_FE)
	    || (lsr & UART_LSR_OE)) {
		MSG(ERR, "LSR: BI=%d, FE=%d, PE=%d, OE=%d, DR=%d\n", (lsr & UART_LSR_BI) >> 4,
		    (lsr & UART_LSR_FE) >> 3, (lsr & UART_LSR_PE) >> 2, (lsr & UART_LSR_OE) >> 1,
		    lsr & UART_LSR_DR);
	}
#endif
...
void mtk_uart_get_modem_status(struct mtk_uart *uart)
{
	...
	MSG(INFO, "MSR: DCD(%d), RI(%d), DSR(%d), CTS(%d)\n",
	    ...
	    status & UART_MSR_RI ? 1 : 0, status & UART_MSR_DSR ? 1 : 0,
...
unsigned int mtk_uart_get_mctrl(struct uart_port *port)
{
	...
	MSG(INFO, "MSR: OUT1(%d), OUT2(%d), LOOP(%d)\n",
	    status & UART_MCR_OUT1 ? 1 : 0, status & UART_MCR_OUT2 ? 1 : 0,
