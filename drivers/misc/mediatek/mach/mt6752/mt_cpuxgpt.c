...
__cpuxgpt0_irq_handler, __cpuxgpt1_irq_handler, __cpuxgpt2_irq_handler, __cpuxgpt3_irq_handler,
__cpuxgpt4_irq_handler, __cpuxgpt5_irq_handler, __cpuxgpt6_irq_handler, __cpuxgpt7_irq_handler,};
...
static void __cpuxgpt_set_clk(unsigned int div)
{
	...
	/* pr_alert("%s fwq  div is  0x%x\n",__func__, div); */
	if (div != CLK_DIV1 && div != CLK_DIV2 && div != CLK_DIV4)
		pr_alert("%s error: div is not right\n", __func__);
...
/* pr_alert("mt_cpuxgpt_init: cpuxgpt_regs=0x%x, irq0=%d, irq1=%d, irq2=%d,*/
/* irq3=%d, irq4=%d, irq5=%d, irq6=%d, irq7=%d\n", */
...
int cpu_xgpt_register_timer(unsigned int id, irqreturn_t(*func) (int irq, void *dev_id))
{
	...
	if (id > 7 || id < 0) {
		pr_alert("%s: err idnumber id=%d should be 0~7\n", __func__, id);
		return -1;
	}
	switch (id) {
	case 0:
	...
	ret =
	    request_irq(irq_id, (irq_handler_t) cpuxgpt_irq_handler[id], IRQF_TRIGGER_HIGH, name,
			NULL);
	if (ret != 0) {
		pr_alert("%s:%s fail to register irq\n", __func__, name);
		return ret;
	}

	pr_alert("%s:%s register irq (%d) ok\n", __func__, name, irq_id);
