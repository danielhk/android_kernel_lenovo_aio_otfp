...
static unsigned int xgpt_boot_up_time(void)
{
	...
	tick = ((tick + (GPT4_1MS_TICK - 1)) / GPT4_1MS_TICK);
	return tick;
}
...
unsigned long long mt_read_sched_clock(void)
{
	...
	cycles = ((unsigned long long)(syscnt_cnt[1])) << 32 | (unsigned long long)(syscnt_cnt[0]);
...
static void __init mt_gpt_init(struct device_node *node)
{
	...
	/* pr_alert("mt_gpt_init: tmr_regs=0x%lx, tmr_irq=%d, freq=0x%lx\n"*/
	/*, xgpt_timers.tmr_regs, xgpt_timers.tmr_irq, freq); */
...
int start_gpt(unsigned int id)
{
	...
	if (!(dev->flags & GPT_IN_USE)) {
		pr_alert("%s: GPT%d is not in use!\n", __func__, id);
		return -EBUSY;
	}
