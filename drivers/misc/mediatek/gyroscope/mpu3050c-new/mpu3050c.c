...
#define MPU3000_DEFAULT_FS		MPU3000_FS_2000
#define MPU3000_DEFAULT_LSB		MPU3000_FS_2000_LSB
...
static struct i2c_board_info i2c_mpu3000 __initdata = { I2C_BOARD_INFO("MPU3000", (0xD0 >> 1)) };
...
{
	obj->cali_sw[MPU3000_AXIS_X] =
	    obj->cvt.sign[MPU3000_AXIS_X] * dat[obj->cvt.map[MPU3000_AXIS_X]];
	obj->cali_sw[MPU3000_AXIS_Y] =
	    obj->cvt.sign[MPU3000_AXIS_Y] * dat[obj->cvt.map[MPU3000_AXIS_Y]];
	obj->cali_sw[MPU3000_AXIS_Z] =
	    obj->cvt.sign[MPU3000_AXIS_Z] * dat[obj->cvt.map[MPU3000_AXIS_Z]];
#if DEBUG
	if (atomic_read(&obj->trace) & GYRO_TRC_CALI) {
		GYRO_LOG("test  (%5d, %5d, %5d) ->(%5d, %5d, %5d)->(%5d, %5d, %5d))\n",
			 obj->cvt.sign[MPU3000_AXIS_X], obj->cvt.sign[MPU3000_AXIS_Y],
			 obj->cvt.sign[MPU3000_AXIS_Z], dat[MPU3000_AXIS_X], dat[MPU3000_AXIS_Y],
			 dat[MPU3000_AXIS_Z], obj->cvt.map[MPU3000_AXIS_X],
			 obj->cvt.map[MPU3000_AXIS_Y], obj->cvt.map[MPU3000_AXIS_Z]);
		GYRO_LOG("write gyro calibration data  (%5d, %5d, %5d)\n",
			 obj->cali_sw[MPU3000_AXIS_X], obj->cali_sw[MPU3000_AXIS_Y],
			 obj->cali_sw[MPU3000_AXIS_Z]);
	}
#endif
	return 0;
...
static int MPU3000_ReadCalibration(struct i2c_client *client, int dat[MPU3000_AXES_NUM])
{
	...
	dat[obj->cvt.map[MPU3000_AXIS_X]] =
	    obj->cvt.sign[MPU3000_AXIS_X] * obj->cali_sw[MPU3000_AXIS_X];
	dat[obj->cvt.map[MPU3000_AXIS_Y]] =
	    obj->cvt.sign[MPU3000_AXIS_Y] * obj->cali_sw[MPU3000_AXIS_Y];
	dat[obj->cvt.map[MPU3000_AXIS_Z]] =
	    obj->cvt.sign[MPU3000_AXIS_Z] * obj->cali_sw[MPU3000_AXIS_Z];
...
static int MPU3000_WriteCalibration(struct i2c_client *client, int dat[MPU3000_AXES_NUM])
{
	...
	if (!obj || !dat) {
		...
		cali[obj->cvt.map[MPU3000_AXIS_X]] =
		    obj->cvt.sign[MPU3000_AXIS_X] * obj->cali_sw[MPU3000_AXIS_X];
		cali[obj->cvt.map[MPU3000_AXIS_Y]] =
		    obj->cvt.sign[MPU3000_AXIS_Y] * obj->cali_sw[MPU3000_AXIS_Y];
		cali[obj->cvt.map[MPU3000_AXIS_Z]] =
		    obj->cvt.sign[MPU3000_AXIS_Z] * obj->cali_sw[MPU3000_AXIS_Z];
...
static int MPU3000_ReadStart(struct i2c_client *client, bool enable)
{
	...
	if (enable) {
		...
		databuf[1] =
		    (MPU3000_FIFO_GYROX_EN | MPU3000_FIFO_GYROY_EN | MPU3000_FIFO_GYROZ_EN);
	} else {
	...
	*datalen = packet_cnt;
	...
	if (packet_cnt && (abs(packet_thresh - packet_cnt) < 4)) {
		...
		for (i = 0; i < packet_cnt; i++) {
			...
			tmp1[MPU3000_AXIS_X] =
			    (s16) ((buf[MPU3000_AXIS_X * 2 + 1]) | (buf[MPU3000_AXIS_X * 2] << 8));
			tmp1[MPU3000_AXIS_Y] =
			    (s16) ((buf[MPU3000_AXIS_Y * 2 + 1]) | (buf[MPU3000_AXIS_Y * 2] << 8));
			tmp1[MPU3000_AXIS_Z] =
			    (s16) ((buf[MPU3000_AXIS_Z * 2 + 1]) | (buf[MPU3000_AXIS_Z * 2] << 8));

			/* remap coordinate// */
			tmp2[obj->cvt.map[MPU3000_AXIS_X]] =
			    obj->cvt.sign[MPU3000_AXIS_X] * tmp1[MPU3000_AXIS_X];
			tmp2[obj->cvt.map[MPU3000_AXIS_Y]] =
			    obj->cvt.sign[MPU3000_AXIS_Y] * tmp1[MPU3000_AXIS_Y];
			tmp2[obj->cvt.map[MPU3000_AXIS_Z]] =
			    obj->cvt.sign[MPU3000_AXIS_Z] * tmp1[MPU3000_AXIS_Z];
			...
			GYRO_LOG("gyro FIFO packet[%d]:[%04X %04X %04X] => [%5d %5d %5d]\n", i,
				 data[3 * i + MPU3000_AXIS_X], data[3 * i + MPU3000_AXIS_Y],
				 data[3 * i + MPU3000_AXIS_Z], data[3 * i + MPU3000_AXIS_X],
				 data[3 * i + MPU3000_AXIS_Y], data[3 * i + MPU3000_AXIS_Z]);
		}
...
static int MPU3000_ReadGyroData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (hwmsen_read_block(client, MPU3000_REG_GYRO_XH, databuf, 6)) {
		...
		obj->data[MPU3000_AXIS_X] =
		    ((s16)
		     ((databuf[MPU3000_AXIS_X * 2 + 1]) | (databuf[MPU3000_AXIS_X * 2] << 8)));
		obj->data[MPU3000_AXIS_Y] =
		    ((s16)
		     ((databuf[MPU3000_AXIS_Y * 2 + 1]) | (databuf[MPU3000_AXIS_Y * 2] << 8)));
		obj->data[MPU3000_AXIS_Z] =
		    ((s16)
		     ((databuf[MPU3000_AXIS_Z * 2 + 1]) | (databuf[MPU3000_AXIS_Z * 2] << 8)));
#if DEBUG
		if (atomic_read(&obj->trace) & GYRO_TRC_RAWDATA) {
			...
			GYRO_LOG("get gyro raw data (0x%08X, 0x%08X, 0x%08X) -> (%5d, %5d, %5d)\n",
				 obj->data[MPU3000_AXIS_X], obj->data[MPU3000_AXIS_Y],
				 obj->data[MPU3000_AXIS_Z], obj->data[MPU3000_AXIS_X],
				 obj->data[MPU3000_AXIS_Y], obj->data[MPU3000_AXIS_Z]);
		}
#endif
		obj->data[MPU3000_AXIS_X] =
		    obj->data[MPU3000_AXIS_X] + obj->cali_sw[MPU3000_AXIS_X];
		obj->data[MPU3000_AXIS_Y] =
		    obj->data[MPU3000_AXIS_Y] + obj->cali_sw[MPU3000_AXIS_Y];
		obj->data[MPU3000_AXIS_Z] =
		    obj->data[MPU3000_AXIS_Z] + obj->cali_sw[MPU3000_AXIS_Z];

		/*remap coordinate */
		data[obj->cvt.map[MPU3000_AXIS_X]] =
		    obj->cvt.sign[MPU3000_AXIS_X] * obj->data[MPU3000_AXIS_X];
		data[obj->cvt.map[MPU3000_AXIS_Y]] =
		    obj->cvt.sign[MPU3000_AXIS_Y] * obj->data[MPU3000_AXIS_Y];
		data[obj->cvt.map[MPU3000_AXIS_Z]] =
		    obj->cvt.sign[MPU3000_AXIS_Z] * obj->data[MPU3000_AXIS_Z];

		/* Out put the degree/second(o/s) */
		data[MPU3000_AXIS_X] =
		    data[MPU3000_AXIS_X] * MPU3000_FS_MAX_LSB / MPU3000_DEFAULT_LSB;
		data[MPU3000_AXIS_Y] =
		    data[MPU3000_AXIS_Y] * MPU3000_FS_MAX_LSB / MPU3000_DEFAULT_LSB;
		data[MPU3000_AXIS_Z] =
		    data[MPU3000_AXIS_Z] * MPU3000_FS_MAX_LSB / MPU3000_DEFAULT_LSB;


	}

	sprintf(buf, "%04x %04x %04x", data[MPU3000_AXIS_X], data[MPU3000_AXIS_Y],
		data[MPU3000_AXIS_Z]);
...
static int MPU3000_PROCESS_SMT_DATA(struct i2c_client *client, short *data)
{
	...
	for (i = 0; i < total_num; i++) {
		xRMS +=
		    (data[MPU3000_AXES_NUM * i + MPU3000_AXIS_X + 2] -
		     xAvg) * (data[MPU3000_AXES_NUM * i + MPU3000_AXIS_X + 2] - xAvg);
		yRMS +=
		    (data[MPU3000_AXES_NUM * i + MPU3000_AXIS_Y + 2] -
		     yAvg) * (data[MPU3000_AXES_NUM * i + MPU3000_AXIS_Y + 2] - yAvg);
		zRMS +=
		    (data[MPU3000_AXES_NUM * i + MPU3000_AXIS_Z + 2] -
		     zAvg) * (data[MPU3000_AXES_NUM * i + MPU3000_AXIS_Z + 2] - zAvg);
	}
...
/* unsigned long arg) */
static long mpu3000_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GYROSCOPE_IOCTL_SMT_DATA:
		...
		GYRO_LOG("gyroscope read data from kernel OK: SMTdata[0]:%d, copied packet:%zu!\n",
			 SMTdata[0], ((SMTdata[0] * MPU3000_AXES_NUM + 2) * sizeof(s16) + 1));
	...
	case GYROSCOPE_IOCTL_SET_CALI:
		...
		if (copy_from_user(&sensor_data, data, sizeof(sensor_data))) {
			...
			cali[MPU3000_AXIS_X] =
			    sensor_data.x * MPU3000_DEFAULT_LSB / MPU3000_FS_MAX_LSB;
			cali[MPU3000_AXIS_Y] =
			    sensor_data.y * MPU3000_DEFAULT_LSB / MPU3000_FS_MAX_LSB;
			cali[MPU3000_AXIS_Z] =
			    sensor_data.z * MPU3000_DEFAULT_LSB / MPU3000_FS_MAX_LSB;
...
static int mpu3000_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->early_drv.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 2,
	    ...
	    obj->early_drv.resume = mpu3000_late_resume, register_early_suspend(&obj->early_drv);
#endif
