...
static int simp_msdc_wait_clk_stable(struct simp_msdc_host *host) {
    void __iomem *base = host->base;
    int retry = 0;
    int cnt = 1000;
...
static void simp_msdc_config_clock(struct simp_msdc_host *host, unsigned int hz)  /* no ddr */
{
    ...
    /* wait clock stable */
	simp_msdc_wait_clk_stable(host);
}
...
{
	if (!clock_is_on(msdc_cg_clk_id[host->mtk_host->id]))
		enable_clock(msdc_cg_clk_id[host->mtk_host->id], "SD");
	else {
		disable_clock(msdc_cg_clk_id[host->mtk_host->id], "SD");
		enable_clock(msdc_cg_clk_id[host->mtk_host->id], "SD");
	}
...
#if MTK_MMC_DUMP_DBG
        if(retry %1000 == 0){
			pr_err("int cmd=0x%x, arg=0x%x, retry=0x%x, intsts=0x%x\n", raw, arg, retry, intsts);
...
#if MTK_MMC_DUMP_DBG
	pr_err("l_start_offset = 0x%x len = %d buf<%p>\n", l_addr, len, l_buf);
#endif
