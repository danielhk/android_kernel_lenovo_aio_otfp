static int mt_ot_stress_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 *error, u8 rw)
{
    ...
    u32 gear_window_size = OT_START_TUNEWND; //In stress test: use small size to guarantee tmo and crc error won't occur
    u32 i;
...
static int mt_ot_tmo_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 *error, u8 rw)
{
	...
	if(ACMDTMO | DATTMO)
	{
		...
		if(ACMD53Done == 1)
		{
			pr_debug("[SDIO_DVT][%d] AUTOCMD53_DONE status error, AUTOCMD53_DONE should NOT be set to 1 when TMO happened\n",
				rw);
			*error = 1;
		}
	}
...
//Write and Read MSDC_DAT_RDDLY0 and SDIO_TUNE_WIND registers
static int mt_ot_gearRegister_test(struct msdc_host *host, u8 rw, u32 eco_ver, u8 *error)
{
    ...
    for(cmdrdly = 0; cmdrdly < 32; cmdrdly++)
    {
		...
		if(cmdrdly < 32)
		{
			...
			if(tmpdly != cmdrdly)
			{
				pr_debug("[SDIO_DVT][%d] PAD_CMD_RXDLY write fail, write = %d, read = %d\n",
					rw, cmdrdly, tmpdly);
				*error = 1;
			}
		}
	...
	for(; dat0rddly < 32; dat0rddly++)
	{
		...
		if(dat0rddly < 32)
		{
			...
			if(tmpdly != rxdly0)
			{
				pr_debug("[SDIO_DVT][%d] DAT_RD_DLY0 write fail, write = %d, read = %d, cmddly = %d\n",
	...
	for(gear_window_size = 0; gear_window_size < 16; gear_window_size++)
	{
		...
		{
			pr_debug("[SDIO_DVT][%d] SDIO_TUNE_WIND write fail, write = %d, read = %d, cmddly = %d, datdly = %d\n",
...
static int mt_ot_outbound_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 i_rw, u32 eco_ver, u8 *error)
{
	...
	u32 gear_window_size = 7; //In out of bound test: try to trigger out of bound interrupt with some desinated delay
	u32 i;
    u32 GearOutBound = 0;
	u32 intsts;
	...
	for(i=0; i<20; i++)
	{
		...
		if(ot_do_command(host, rawcmd, rawarg, &intsts))
		{
			pr_debug("[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n",
				i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i], dat3rddly[i],
				gear_window_size);
			*error = 1;
			return -1;
		}
		...
		if(cmdrdly[i] > 31 || dat0rddly[i] > 31 || dat1rddly[i] > 31 || dat2rddly[i] > 31 || dat3rddly[i] > 31)
		{
			if(GearOutBound == 0)
			{
				...
				pr_debug("[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 1 when gear is out of bound\n",
					i_rw);
				pr_debug("[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n",
					i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i], dat3rddly[i],
					gear_window_size);
				...
				dat3rddly[i] + gear_window_size > 31 || dat3rddly[i] < gear_window_size )
		{
			...
			{
				pr_debug("[SDIO_DVT]intsts = 0x%x, GearOutBound = %d\n", intsts, GearOutBound);
				pr_debug("[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 1 when gear +/- gear window is out of bound\n",
					i_rw);
				pr_debug("[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n",
					i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i], dat3rddly[i],
					gear_window_size);
		...
		else
		{
			...
			{
				pr_debug("[SDIO_DVT]intsts = 0x%x, GearOutBound = %d\n", intsts, GearOutBound);
				pr_debug("[SDIO_DVT][%d] GEAR_OUT_BOUND status error, GEAR_OUT_BOUND should be set to 0 when gear is in bound\n",
					i_rw);
				pr_debug("[SDIO_DVT][%d] cmdrdly = %d, dat0rddly = %d, dat1rddly = %d, dat2rddly = %d, dat3rddly = %d, gear_window_size = %d\n\n",
					i_rw, cmdrdly[i], dat0rddly[i], dat1rddly[i], dat2rddly[i], dat3rddly[i],
					gear_window_size);
...
static int mt_ot_crcerr_acmd53_test(struct msdc_host *host, u32 rawcmd, u32 rawarg, u8 rw, u32 eco_ver, u8 *error)
{
    ...
    u32 gear_window_size = 15; //In crc error test: use large window size to cover all delay
	u32 i;
	u32 CRCStatus_CMD=0;
	u32 CRCErr = 0;
	u32 ACMD53Err = 0;
	...
	for (i=0; i<20; i++) {
		...
		/*pr_debug("[SDIO_DVT]MASK_ACMD53_CRC_ERR_INTR = 0x%x, ACMD53_FAIL_ONE_SHOT = 0x%x\n",
			temp0, temp1);*/
		...
		/*pr_debug("[SDIO_DVT]D_DLYLINE_SEL = 0x%x, R_D_SMPL_SEL = 0x%x, W_D_SMPL_SEL = 0x%x\n",
			temp0, temp1, temp2);*/
		...
		if(CRCStatus_CMD != goldenCRC)
		{
			...
			if(CRCErr == 0)
			{
				pr_debug("[SDIO_DVT][%d] SD_AUTOCMD_RESP_CRCERR status error, SD_AUTOCMD_RESP_CRCERR should be set to 1 when CRC error happened\n",
					rw);
				pr_debug("[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n",
			...
			if(ACMD53Err == 0)
			{
				pr_debug("[SDIO_DVT][%d] AUTOCMD53_FAIL status error, AUTOCMD53_FAIL should be set to 1 when CRC error happened\n",
					rw);
				pr_debug("[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n",
					rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
				pr_debug("[SDIO_DVT]intsts = 0x%x\n", intsts);
		...
		{
			/*pr_debug("[SDIO_DVT][%d] AUTOCMD53 CMD CRC is 0\n", rw);*/
			/*pr_debug("[SDIO_DVT][%d] cmd crc status = %d, window size = %d, cmddly = %d, datdly = %d\n",
				rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);*/
			...
			if(CRCErr)
			{
				pr_debug("[SDIO_DVT][%d] SD_AUTOCMD_RESP_CRCERR status error, SD_AUTOCMD_RESP_CRCERR should be set to 0 when N0 CRC error\n",
					rw);
				pr_debug("[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n",
					rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
				pr_debug("[SDIO_DVT]intsts = 0x%x\n", intsts);
			...
			if(ACMD53Err)
			{
				pr_debug("[SDIO_DVT][%d] AUTOCMD53_FAIL status error, AUTOCMD53_FAIL should be set to 0 when No CRC error\n",
					rw);
				pr_debug("[SDIO_DVT][%d] cmd crc status = 0x%x, gear window size = %d, cmddly = %d, datdly = %d\n",
					rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);
				pr_debug("[SDIO_DVT]intsts = 0x%x\n", intsts);
		   ...
		   cmd->resp[3] != goldenCRC )
		{
			...
			if(CRCErr == 0)
			{
				pr_debug("[SDIO_DVT][%d] SD_DATA_CRCERR status error, SD_DATA_CRCERR should be set to 1 when CRC error happened\n",
					rw);
				pr_debug("[SDIO_DVT][%d] dat0 crc status = 0x%x, dat1 crc status = 0x%x, dat2 crc status = 0x%x, dat3 crc status = 0x%x\n",
					rw, cmd->resp[0], cmd->resp[1], cmd->resp[2], cmd->resp[3]);
				pr_debug("[SDIO_DVT][%d] gear window size = %d, cmddly = %d, datdly = %d\n",
		...
		{
			/*pr_debug("[SDIO_DVT][%d] AUTOCMD53 DAT CRC is 0\n", rw);*/
			/*pr_debug("[SDIO_DVT][%d] cmd crc status = %d, window size = %d, cmddly = %d, datdly = %d\n",
				rw, CRCStatus_CMD, gear_window_size, cmdrdly[i], rxdly0);*/
			//sdr_get_field(MSDC_INT, MSDC_INT_DATCRCERR, CRCErr);	// Get dat crc err
			//sdr_get_field(MSDC_INT, MSDC_INT_ACMD53_FAIL, ACMD53Err);	// Get ACMD53 error flag
			...
			if(CRCErr)
			{
				pr_debug("[SDIO_DVT][%d] SD_DATA_CRCERR status error, SD_DATA_CRCERR should be set to 0 when No CRC error\n",
					rw);
				pr_debug("[SDIO_DVT][%d] dat0 crc status = 0x%x, dat1 crc status = 0x%x, dat2 crc status = 0x%x, dat3 crc status = 0x%x\n",
					rw, cmd->resp[0], cmd->resp[1], cmd->resp[2], cmd->resp[3]);
				pr_debug("[SDIO_DVT][%d] gear window size = %d, cmddly = %d, datdly = %d\n",
		...
		if(ACMD53Done == 0)
		{
			pr_debug("[SDIO_DVT][%d] AUTOCMD53_DONE status error, AUTOCMD53_DONE should be set to 1 at the end of training blocks\n",
				rw);
			pr_debug("[SDIO_DVT][%d] gear window size = %d, cmddly = %d, datdly = %d\n",
				rw, gear_window_size, cmdrdly[i], rxdly0);
			pr_debug("[SDIO_DVT]intsts = 0x%x\n", intsts);
...
#if 0 //Maybe the following sections can be removed.
