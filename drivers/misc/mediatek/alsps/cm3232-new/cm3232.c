...
static struct i2c_board_info __initdata i2c_cm3232 = {
    I2C_BOARD_INFO(CM3232_DEV_NAME, (0x20 >> 1)) };
/*----------------------------------------------------------------------------*/
...
int cm3232_enable_als(struct i2c_client *client, int enable)
{
	...
	if (enable == 1) {
		APS_DBG("cm3232_enable_als enable_als\n");
		...
		APS_DBG("CM3232_REG_ALS_CONF evalue value_low = %x, value_high = %x\n", databuf[0],
			databuf[1]);
#endif
		...
		APS_DBG("cm3232_enable_als disable_als\n");
#if 0				/* modify for cm3232 speed up report rate.131012 xgt */
		databuf[0] = CM3232_REG_ALS_CONF;
		res = i2c_master_send(client, databuf, 0x201);
		if (res < 0) {
		...
		APS_DBG("CM3232_REG_ALS_CONF dvalue value_low = %x, value_high = %x\n", databuf[0],
			databuf[1]);
#endif
...
long cm3232_read_als(struct i2c_client *client, u16 *data)
{
	...
	APS_DBG("CM3232_REG_ALS_DATA value value_low = %x, value_high = %x\n", databuf[0],
		databuf[1]);
...
static ssize_t cm3232_show_status(struct device_driver *ddri, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "REGS: %02X %02lX %02lX\n",
			atomic_read(&cm3232_obj->als_cmd_val), cm3232_obj->enable,
			cm3232_obj->pending_intr);

	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "MISC: %d\n",
...
/*----------------------------------------------------------------------------*/
static int read_int_from_buf(struct cm3232_priv *obj, const char *buf, size_t count, u32 data[],
			     int len)
{
	int idx = 0;
...
static ssize_t cm3232_store_alslv(struct device_driver *ddri, const char *buf, size_t count)
{
	if (!cm3232_obj) {
		...
		memcpy(cm3232_obj->als_level, cm3232_obj->hw->als_level,
		   ...
		   read_int_from_buf(cm3232_obj, buf, count, cm3232_obj->hw->als_level,
...
static ssize_t cm3232_store_alsval(struct device_driver *ddri, const char *buf, size_t count)
{
	if (!cm3232_obj) {
		...
		memcpy(cm3232_obj->als_value, cm3232_obj->hw->als_value,
		   ...
		   read_int_from_buf(cm3232_obj, buf, count, cm3232_obj->hw->als_value,
...
static int cm3232_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		if ((err = driver_create_file(driver, cm3232_attr_list[idx]))) {
			APS_ERR("driver_create_file (%s) = %d\n", cm3232_attr_list[idx]->attr.name,
				err);
			break;
		}
	}
...
static int cm3232_check_intr(struct i2c_client *client)
{
	...
	APS_DBG("CM3232_REG_PS_DATA value value_low = %x, value_reserve = %x\n", databuf[0],
		databuf[1]);
	...
	APS_DBG("CM3232_REG_INT_FLAG value value_low = %x, value_high = %x\n", databuf[0],
		databuf[1]);
...
static void cm3232_eint_work(struct work_struct *work)
{
	struct cm3232_priv *obj =
	    (struct cm3232_priv *)container_of(work, struct cm3232_priv, eint_work);
...
int cm3232_setup_eint(struct i2c_client *client)
{
	...
	mt65xx_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_DEBOUNCE_EN,
				 CUST_EINT_ALS_POLARITY, cm3232_eint_func, 0);
...
static void cm3232_init_done_work(struct work_struct *work)
{
	...
	max_cust_data_size_per_packet =
	    sizeof(data.set_cust_req.custData) - offsetof(CM3232_SET_CUST, data);
	...
	for (i = 0; sizeOfCustData > 0; i++) {
		...
		len +=
		    offsetof(SCP_SENSOR_HUB_SET_CUST_REQ, custData) + offsetof(CM3232_SET_CUST,
									       data);
...
static int cm3232_als_enable_nodata(int en)
{
	...
	APS_DBG("cm3232_obj als enable value = %d\n", en);
...
static int cm3232_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->early_drv.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING - 2,
	    ...
	    obj->early_drv.resume = cm3232_late_resume, register_early_suspend(&obj->early_drv);
#endif
	cm3232_init_flag = 0;
	APS_DBG("cm3232_i2c_probe OK\n");
	APS_DBG("%s: OK\n", __func__);
