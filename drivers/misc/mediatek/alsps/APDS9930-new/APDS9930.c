...
static struct i2c_board_info i2c_APDS9930 __initdata = { I2C_BOARD_INFO("APDS9930", 0x39) };
...
int APDS9930_i2c_master_operate(struct i2c_client *client, const char *buf, int count, int i2c_flag)
{
	...
	case I2C_FLAG_READ:
		...
		break;
	default:
		APS_DBG("APDS9930_i2c_master_operate i2c_flag command not support!\n");
		break;
	}
	if (res <= 0) {
		goto EXIT_ERR;
...
static long APDS9930_enable_als(struct i2c_client *client, int enable)
{
	...
	/* APS_DBG("APDS9930_CMM_ENABLE als value = %x\n",databuf[0]); */

	if (enable) {
		databuf[1] = databuf[0] | 0x03;
		databuf[0] = APDS9930_CMM_ENABLE;
		/* APS_DBG("APDS9930_CMM_ENABLE enable als value = %x\n",databuf[1]); */
		...
		databuf[0] = APDS9930_CMM_ENABLE;
		/* APS_DBG("APDS9930_CMM_ENABLE disable als value = %x\n",databuf[1]); */
		res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
		if (res <= 0) {
			goto EXIT_ERR;
		}
...
static long APDS9930_enable_ps(struct i2c_client *client, int enable)
{
	...
	/* APS_DBG("APDS9930_CMM_ENABLE ps value = %x\n",databuf[0]); */

	if (enable) {
		databuf[1] = databuf[0] | 0x05;
		databuf[0] = APDS9930_CMM_ENABLE;
		/* APS_DBG("APDS9930_CMM_ENABLE enable ps value = %x\n",databuf[1]); */
		res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
		if (res <= 0) {
			goto EXIT_ERR;
		}
		...
		databuf[0] = APDS9930_CMM_ENABLE;
		/* APS_DBG("APDS9930_CMM_ENABLE disable ps value = %x\n",databuf[1]); */
		res = APDS9930_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
		if (res <= 0) {
			goto EXIT_ERR;
		}
...
int APDS9930_read_als(struct i2c_client *client, u16 *data)
{
	...
	/* APS_DBG("APDS9930_read_als als_value_lux = %d\n", *data); */
	return 0;
...
long APDS9930_read_ps(struct i2c_client *client, u16 *data)
{
	...
	/* APS_DBG("yucong APDS9930_read_ps ps_data=%d, low:%d  high:%d", *data, buffer[0], buffer[1]); */
...
static int APDS9930_get_ps_value(struct APDS9930_priv *obj, u16 ps)
{
	...
	if (ps_cali.valid == 1) {
		...
		APS_DBG("APDS9930_get_ps_value val  = %d", val);
	} else {
...
static void APDS9930_eint_work(struct work_struct *work)
{
	struct APDS9930_priv *obj =
	    (struct APDS9930_priv *)container_of(work, struct APDS9930_priv, eint_work);
	int err;
	...
	if ((err = APDS9930_check_intr(obj->client))) {
		...
		APS_DBG("APDS9930_eint_work rawdata ps=%d als_ch0=%d!\n", obj->ps, obj->als);
		APS_DBG("APDS9930 int top half time = %lld\n", int_top_time);

		if (obj->als > 40000) {
			APS_DBG("APDS9930_eint_work ALS too large may under lighting als_ch0=%d!\n",
				obj->als);
			return;
		}
		...
		APS_DBG("APDS9930_eint_work APDS9930_CMM_ENABLE ps value = %x\n", databuf[0]);
		...
		APS_DBG
		    ("APDS9930_eint_work APDS9930_CMM_INT_LOW_THD_LOW before databuf[0]=%d databuf[1]=%d!\n",
		     databuf[0], databuf[1]);
		...
		APS_DBG
		    ("APDS9930_eint_work APDS9930_CMM_INT_HIGH_THD_LOW before databuf[0]=%d databuf[1]=%d!\n",
		     databuf[0], databuf[1]);
#endif
/*singal interrupt function add*/
		...
		APS_DBG
		    ("APDS9930_eint_work APDS9930_CMM_INT_LOW_THD_LOW after databuf[0]=%d databuf[1]=%d!\n",
		     databuf[0], databuf[1]);
		...
		APS_DBG
		    ("APDS9930_eint_work APDS9930_CMM_INT_HIGH_THD_LOW after databuf[0]=%d databuf[1]=%d!\n",
		     databuf[0], databuf[1]);
#endif
...
static int APDS9930_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	APS_DBG("APDS9930_init_client() OK!\n");
