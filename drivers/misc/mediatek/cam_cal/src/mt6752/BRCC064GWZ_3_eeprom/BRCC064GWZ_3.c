...
u16 i2cId);
extern int iBurstReadRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u8 *a_pRecvData, u16 a_sizeRecvData,
u16 i2cId);
		...
		do {
			if (16 <= loop[loopCount]) {
				EEPROMDB("[BRCC064GWZ_3_eeprom]1 loopCount=%d loop[loopCount]=%d puSendCmd[0]=%x
				 puSendCmd[1]=%x, EEPROM(%x)\n", loopCount , loop[loopCount], puSendCmd[0],
				...
				EEPROMDB("[BRCC064GWZ_3_eeprom]2 loopCount=%d loop[loopCount]=%d
				 puSendCmd[0]=%x puSendCmd[1]=%x\n", loopCount ,
...
int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata)
{
	...
	/* EEPROMDB("[S24EEPORM] iReadData finial address is %d length is
...
if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
	...
	EEPROMDB("[BRCC064GWZ_3_eeprom] init Working buffer address 0x%8x
