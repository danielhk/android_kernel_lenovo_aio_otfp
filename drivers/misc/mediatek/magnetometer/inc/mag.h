...
#define EVENT_TYPE_MAGEL_STATUS     ABS_MISC
...
#define EVENT_TYPE_GYRO_X          	ABS_RUDDER
#define EVENT_TYPE_GYRO_Y          	ABS_WHEEL
#define EVENT_TYPE_GYRO_Z          	ABS_GAS
#define EVENT_TYPE_GYRO_STATUS     	ABS_BRAKE

#define EVENT_TYPE_GRAVITY_X        ABS_HAT0X
#define EVENT_TYPE_GRAVITY_Y        ABS_HAT0Y
#define EVENT_TYPE_GRAVITY_Z        ABS_HAT1X
#define EVENT_TYPE_GRAVITY_STATUS   ABS_HAT1Y

#define EVENT_TYPE_LACCEL_X         ABS_HAT2X
#define EVENT_TYPE_LACCEL_Y         ABS_HAT2Y
#define EVENT_TYPE_LACCEL_Z         ABS_HAT3X
#define EVENT_TYPE_LACCEL_STATUS    ABS_HAT3Y

#define EVENT_TYPE_ROTVEC_X         ABS_PRESSURE
#define EVENT_TYPE_ROTVEC_Y         ABS_DISTANCE
#define EVENT_TYPE_ROTVEC_Z         ABS_TILT_X
#define EVENT_TYPE_ROTVEC_W        ABS_TILT_Y
#define EVENT_TYPE_ROTVEC_STATUS     ABS_TOOL_WIDTH
#endif
...
#ifdef AKM_Pseudogyro
#define MAX_CHOOSE_G_NUM 6
#define MAX_M_V_SENSOR  6
#else
...
#ifdef AKM_Pseudogyro
#define ID_M_V_ROTATION_VECTOR 2
#define ID_M_V_LINEAR_ACC 3
#define ID_M_V_GRAVITY 4
#define ID_M_V_SOFT_GYRO 5
#endif
...
#ifdef AKM_Pseudogyro
	int (*get_data_gs)(int *x,int *y, int *z,int *status);
	int (*get_data_rv)(int *x,int *y, int *z,int *status);
	int (*get_data_gr)(int *x,int *y, int *z,int *status);
	int (*get_data_la)(int *x,int *y, int *z,int *status);
#endif
...
struct mag_control_path
{
	...
	int (*o_enable)(int en);
#ifdef AKM_Pseudogyro
	int (*gs_open_report_data)(int en);
	int (*gs_set_delay)(u64 delay);
	int (*gs_enable)(int en);
	int (*rv_open_report_data)(int en);
	int (*rv_set_delay)(u64 delay);
	int (*rv_enable)(int en);
	int (*gr_open_report_data)(int en);
	int (*gr_set_delay)(u64 delay);
	int (*gr_enable)(int en);
	int (*la_open_report_data)(int en);
	int (*la_set_delay)(u64 delay);
	int (*la_enable)(int en);
#endif
...
struct mag_context {
	...
	uint32_t is_first_data_after_enable;
	bool is_polling_run;
