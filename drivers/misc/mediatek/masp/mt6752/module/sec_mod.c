...
if (!es_base) {
	pr_debug(KERN_ERR "[%s] ES register remapping failed\n", SEC_DEV_NAME);
	return -ENXIO;
}
return 0;
}
...
if (!hacc_base) {
	pr_debug(KERN_ERR "[%s] HACC register remapping failed\n", SEC_DEV_NAME);
	return -ENXIO;
}
...
if (ret) {
	pr_debug(KERN_ERR "[%s] Regist Failed (%d)\n", SEC_DEV_NAME, ret);
	return ret;
}

sec_class = class_create(THIS_MODULE, SEC_DEV_NAME);
if (NULL == sec_class) {
	pr_debug(KERN_ERR "[%s] Create class failed(0x%x)\n", SEC_DEV_NAME, ret);
	ret = -1;
	return ret;
}
...
if (NULL == sec_class) {
	pr_debug(KERN_ERR "[%s] Create device failed(0x%x)\n", SEC_DEV_NAME, ret);
...
if (ret) {
	pr_debug(KERN_ERR "[ES] Reg platform driver failed (%d)\n", ret);
	return ret;
}

ret = platform_driver_register(&masp_driver);
if (ret) {
	pr_debug(KERN_ERR "[%s] Reg platform driver failed (%d)\n", SEC_DEV_NAME, ret);
	return ret;
}
