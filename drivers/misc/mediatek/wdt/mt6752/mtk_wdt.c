...
#define REG_ADDR(x)             ((unsigned int *)(msdc_base + OFFSET_##x))
#define OFFSET_EMMC_IOCON       (0x7c)
#define EMMC_IOCON              REG_ADDR(EMMC_IOCON)
#define MSDC_SET_BIT32(addr, mask)	(*(unsigned int *)(addr) |= (mask));
#define MSDC_CLR_BIT32(addr, mask)	(*(unsigned int *)(addr) &= ~(mask));
...
void wdt_arch_reset(char mode)
{
	...
	pr_debug("wdt_arch_reset read MT6325 0xA4A =%x\n", A4A_rdata);
