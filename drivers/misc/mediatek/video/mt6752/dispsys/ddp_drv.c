...
static const struct mc_uuid_t MC_UUID_TPLAY = { {0x05, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
#endif				/* defined(CONFIG_TRUSTONIC_TEE_SUPPORT) &&
...
static int disp_probe(struct platform_device *pdev)
{
	...
	/*size = sizeof(struct dispsys_device) * new_count;*/
	...
	for (i = 0; i < DISP_REG_NUM; i++) {
		...
		if (i < DISP_REG_CONFIG && (i != DISP_REG_PWM)) {
			...
			dispsys_irq[i] = (unsigned int)dispsys_dev->irq[i];
