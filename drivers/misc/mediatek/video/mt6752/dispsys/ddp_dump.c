		...
		if (DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)) != 0 &&
		   ((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_EN(i)+0x20*i) == 1 &&
			...
			DISP_REG_GET(DISP_REG_CONFIG_MUTEX_SOF(i)+0x20*i) == SOF_SINGLE)) {
				...
				i, ddp_get_fmt_name(DISP_MODULE_MUTEX, DISP_REG_GET(DISP_REG_CONFIG_MUTEX_SOF(i))));
				p += len;
			for (j = 6; j <= 18; j++) {
...
static void  mmsys_config_dump_analysis(void)
{
	...
	DDPDUMP("larb0_force_on=%d, smi_common_force_on=%d\n",
...
static void dsi_dump_reg(DISP_MODULE_ENUM module)
{
	...
	if (DISP_MODULE_DSI0) {
		...
		for (i = 0; i < 25*16; i += 16) {
			...
			i, INREG32(DISPSYS_DSI0_BASE + i), INREG32(DISPSYS_DSI0_BASE + i + 0x4),
			INREG32(DISPSYS_DSI0_BASE + i + 0x8), INREG32(DISPSYS_DSI0_BASE + i + 0xc));
		}
		DDPDUMP("DSI0 CMDQ+0x200 : 0x%08x  0x%08x  0x%08x  0x%08x\n",
			INREG32(DISPSYS_DSI0_BASE + 0x200), INREG32(DISPSYS_DSI0_BASE + 0x200 + 0x4),
			INREG32(DISPSYS_DSI0_BASE + 0x200 + 0x8), INREG32(DISPSYS_DSI0_BASE + 0x200 + 0xc));

		DDPDUMP("==DISP MIPI REGS==\n");
		for (i = 0; i < 10*16; i += 16) {
			DDPDUMP("MIPI+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n",
			i, INREG32(MIPITX_BASE + i), INREG32(MIPITX_BASE + i + 0x4),
		...
		for (i = 0; i < 20*16; i += 16) {
			...
			i, INREG32(0xF401c000 + i), INREG32(0xF401c000 + i + 0x4),
		...
		for (i = 0; i < 20*16; i += 16) {
			...
			i, INREG32(0xF401b000 + i), INREG32(0xF401b000 + i + 0x4),
