...
#define dprec_dump_max_length (1024*16*8)
...
void dprec_logger_reset(DPREC_LOGGER_ENUM source)
{
	...
	l->count, l->period_frame, l->period_max_frame, l->period_min_frame,
	l->ts_start, l->ts_trigger); */
}
...
int dprec_logger_get_result_string(DPREC_LOGGER_ENUM source, char *stringbuf, int strlen)
{
	...
	/* len += scnprintf(stringbuf+len, strlen - len, "[DPREC][LOGGER]%s, count=%d, avg count=%d.%02dfps,
		avg period=%lld.%ludms, max=%d.%02dms, min=%lld.%lums\n", dprec_logger_spy(source), l->count, fps/100,
		fps%100,SPLIT_NS(l->period_max_frame), max/100, max%100, SPLIT_NS(l->period_min_frame)); */
				  ...
				  unsigned long long *pmax, unsigned long long *pmin)
{
	...
	l->count, fps/100, fps%100,SPLIT_NS(l->period_max_frame), max/100, max%100, SPLIT_NS(l->period_min_frame)); */
	/* len += scnprintf(stringbuf+len, strlen - len, "[DPREC][LOGGER]%s\n\tcount = %8llu, fps=%3llu.%02lu,
	avg=%8llu.%02lums, max=%8llu.%02lums, min=%8llu.%02lums\n", dprec_logger_spy(source),
	l->count, fps_high, fps_low, SPLIT_NS(avg), SPLIT_NS(l->period_max_frame), SPLIT_NS(l->period_min_frame)); */
...
{   
    ...
    if(analysize_length>=dprec_dump_max_length-512)
    {
        analysize_length = 0;
    }
    ...
    analysize_length +=
	    scnprintf(dprec_string_buffer_analysize + analysize_length,
		      dprec_dump_max_length - analysize_length, "[%5lu.%06lu]:%s",
		      (unsigned long)time, rem_nsec/1000, string);
	return;
}
...
int dprec_mmp_dump_ovl_layer(void *ovl_layer, unsigned int l, unsigned int session)
{
	if (gCapturePriLayerEnable) {
		if (gCapturePriLayerNum >= HW_OVERLAY_COUNT) {
			ddp_mmp_ovl_layer((OVL_CONFIG_STRUCT *)ovl_layer, gCapturePriLayerDownX,
					gCapturePriLayerDownY, session);
		} else if (gCapturePriLayerNum == l) {
			ddp_mmp_ovl_layer((OVL_CONFIG_STRUCT *)ovl_layer, gCapturePriLayerDownX, gCapturePriLayerDownY,
					  session);
		}
		return 0;
	}
...
int dprec_mmp_dump_ovl_layer(void *ovl_layer, unsigned int l,
