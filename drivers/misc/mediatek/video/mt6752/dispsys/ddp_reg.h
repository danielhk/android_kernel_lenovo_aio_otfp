...
#define DISP_CPU_REG_SET_FIELD(field, reg32, val)  \
	do {\
		...
		mt_reg_sync_writel(((unsigned int)(*(volatile unsigned long*)(reg32)) & \
	~REG_FLD_MASK(field))|REG_FLD_VAL((field), (val)), (volatile unsigned long*)reg32);  \
	} while (0)
...
#define DISP_REG_SET_FIELD(handle, field, reg32, val)  \
	do {  \
		...
		if (handle == 0) { \
			mt_reg_sync_writel((__raw_readl(reg32) & ~REG_FLD_MASK(field))|REG_FLD_VAL((field), \
				(val)), (reg32));  \
		} else { \
...
#define DISP_REG_SET_FIELD_PA(handle, field, reg32, val)  \
	do {  \
		if (handle == 0) { \
			ASSERT(0); \
		} else { \
			cmdqRecWrite(handle, reg32, val<<REG_FLD_SHIFT(field), REG_FLD_MASK(field)); \
		} \
	} while (0)
#endif
...
/*#define DISP_REG_VENCPLL_CON00xf0209260  /* bit0 1*/
...
#define DITHER_CFG_FLD_CHKSUM_SE	REG_FLD(2, 29)
