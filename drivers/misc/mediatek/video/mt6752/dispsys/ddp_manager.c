...
static DDP_IRQ_EVENT_MAPPING ddp_irq_event_list[DEFAULT_IRQ_EVENT_SCENARIO][DISP_PATH_EVENT_NUM] = {
	{			/* ovl0 path */
	 ...
	 {DDP_IRQ_UNKNOW}, /*TRIGER*/ {DDP_IRQ_AAL_OUT_END_FRAME},	/*AAL_OUT_END_EVENT */
	 },
	{			/* ovl1 path */
	 ...
	 {DDP_IRQ_UNKNOW}, /*TRIGER*/ {DDP_IRQ_UNKNOW},	/*AAL_OUT_END_EVENT */
	 },
	{			/* rdma path */
...
{
	if (module == DISP_MODULE_DSI0)   /*bypass dsi */
		return 0;
...
int dpmgr_path_config(disp_path_handle dp_handle, disp_ddp_path_config *config, void *cmdq_handle)
{
	...
	if ((config->ovl_dirty == 1) && (ovl_get_status() == DDP_OVL1_STATUS_IDLE)) {
		...
		if(isAEEEnabled && ovl_get_status() == DDP_OVL1_STATUS_PRIMARY) {
			assert_layer = primary_display_get_option("ASSERT_LAYER");
			if(config->ovl_config[assert_layer].addr == get_Assert_Layer_PA()) {
				config->ovl_config[assert_layer].layer_en = 1;
				DDPMSG("assert layer after cascade: ovl_config[%d] :layer_en: %d, address: 0x%lx\n",
					assert_layer, config->ovl_config[assert_layer].layer_en, config->ovl_config[assert_layer].addr);
				}
			}
		...
		if (enable_layers <= 4) {
			...
			if(isAEEEnabled && (config->ovl_config[OVL_LAYER_NUM - 1].addr == get_Assert_Layer_PA())) {
				assert_layer = primary_display_get_option("ASSERT_LAYER");
				config->ovl_config[assert_layer] = config->ovl_config[OVL_LAYER_NUM - 1];
				config->ovl_config[assert_layer].layer_en = 1;
				config->ovl_config[assert_layer].layer = assert_layer;
				DDPMSG("assert layer after disable cascade: ovl_config[%d] :layer_en: %d, address: 0x%lx\n",
					assert_layer, config->ovl_config[assert_layer].layer_en, config->ovl_config[assert_layer].addr);
				}
		} else {
...
int dpmgr_path_build_cmdq(disp_path_handle dp_handle, void *trigger_loop_handle, CMDQ_STATE state)
{
	...
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if ((module_name >= DISP_MODULE_OVL0 && module_name < DISP_MODULE_NUM)
		    && ddp_modules_driver[module_name] != 0) {
	...
	if (wq_handle->init) {
		...
		if (ret == 0) {
			...
			MMProfileLogEx(ddp_mmp_get_events()->dpmgr_wait_event_timeout,
				       MMProfileFlagPulse, ddp_get_scenario_name(handle->scenario),
				       event);
		...
		if (regvalue & (0x1 << i)) {
			...
			for (j = 0; j < DISP_PATH_EVENT_NUM; j++) {
				if (handle->wq_list[j].init
					...
					/* MMProfileLogEx(ddp_mmp_get_events()->primary_wakeup, MMProfileFlagPulse,
									j, irq_bit); */
				}
			}
		}
