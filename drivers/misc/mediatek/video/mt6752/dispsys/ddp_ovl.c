...
static int ovl_layer_config(DISP_MODULE_ENUM module, unsigned int layer,
				unsigned int source, DpColorFormat format, unsigned long addr, unsigned int src_x,
				/* ROI x offset */
			    ...
			    DISP_BUFFER_TYPE sec, unsigned int is_engine_sec, void *handle)
{
	...
	if (fmt == OVL_INPUT_FORMAT_VYUY || fmt == OVL_INPUT_FORMAT_YVYU
	    || fmt == OVL_INPUT_FORMAT_UYVY || fmt == OVL_INPUT_FORMAT_YUYV) {
		if (src_x % 2 != 0) {
			DDPERR("OVL para invalid, fmt=%s, src_x=%d.\n",
			       ovl_intput_format_name(fmt, input_swap), src_x);
			DDPAEE("OVL para invalid, fmt=%s, src_x=%d.\n",
			       ovl_intput_format_name(fmt, input_swap), src_x);
		}
		if (dst_w % 2 != 0) {
			DDPERR("OVL para invalid, fmt=%s, dst_w=%d.\n",
			       ovl_intput_format_name(fmt, input_swap), dst_w);
			DDPAEE("OVL para invalid, fmt=%s, dst_w=%d.\n",
			       ovl_intput_format_name(fmt, input_swap), dst_w);
		}
	}
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) && defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
...
void ovl_get_info(int idx, void *data)
{
	...
	if (ovl_status == DDP_OVL1_STATUS_SUB || ovl_status == DDP_OVL1_STATUS_IDLE) {
...
static int ovl_config_l(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *handle)
{
	...
	for (i = layer_min; i < layer_max; i++) {
		if (pConfig->ovl_config[i].layer_en != 0) {
			...
			ovl_layer_config(module, i % 4, pConfig->ovl_config[i].source, pConfig->ovl_config[i].fmt,
			pConfig->ovl_config[i].addr, pConfig->ovl_config[i].src_x, pConfig->ovl_config[i].src_y,
			pConfig->ovl_config[i].src_pitch, pConfig->ovl_config[i].dst_x, pConfig->ovl_config[i].dst_y,
			pConfig->ovl_config[i].dst_w, pConfig->ovl_config[i].dst_h, pConfig->ovl_config[i].keyEn,
			pConfig->ovl_config[i].key, pConfig->ovl_config[i].aen, pConfig->ovl_config[i].alpha,
			pConfig->ovl_config[i].sur_aen, pConfig->ovl_config[i].src_alpha,
