...
static ssize_t fbconfig_write(struct file *file, const char __user *ubuf, size_t count, loff_t *ppos)
{
	return 0;
}
...
/* LCM_DRIVER*pLcm_drv=pm->pLcm_drv; */
	...
	case DRIVER_IC_CONFIG:
		{
			int i;
			/*(CONFIG_RECORD_LIST *) kmalloc(sizeof(CONFIG_RECORD_LIST), GFP_KERNEL);*/
			CONFIG_RECORD_LIST *record_tmp_list =
				kmalloc(sizeof(CONFIG_RECORD_LIST), GFP_KERNEL);
...
/*		pr_debug("add cmd:type:%d num:%d value:\r\n",record_tmp_list->record.type,
record_tmp_list->record.ins_num);
...
*/
	...
	case FB_LAYER_GET_EN:
		{
			...
			ovl_all[0].layer_en, ovl_all[1].layer_en, ovl_all[2].layer_en,
			ovl_all[3].layer_en);
	...
	case FB_LAYER_DUMP:
		{
			...
			pr_debug("layer_size=%d, src_pitch=%d, h=%d, mva=0x%x,\n", layer_size,
				ovl_all[global_layer_id].src_pitch, ovl_all[global_layer_id].src_h, mva);
	...
	case FB_GET_MISC:
	{
		...
		*/
		unsigned int a = TOTAL_OVL_LAYER_NUM << 1;
