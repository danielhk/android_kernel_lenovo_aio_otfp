		    ...
		    disp_internal_buffer_info *buf)
{
	if (ctx && head && buf) {
		...
		DISPMSG("enqueue_buffer, head=0x%p, buf=0x%p, mva=0x%08x\n", head, buf, buf->mva);
	}
}
...
void reset_buffer(display_primary_path_context *ctx, disp_internal_buffer_info *buf)
{
	if (ctx && buf) {
		...
		DISPMSG("reset_buffer, buf=0x%p, mva=0x%08x\n", buf, buf->mva);
	}
}
					  ...
					  struct list_head *head)
{
	...
	if (ctx && head) {
		if (!list_empty(head)) {
			...
			DISPMSG("dequeue_buffer, head=0x%p, buf=0x%p, mva=0x%08x\n",
			head, temp, temp->mva);
			list_del_init(&temp->list);

			/*if (list_empty(head))*/
				DISPMSG("after dequeue_buffer, head:0x%p is empty\n", head);
		} else {
	...
	if (ctx && head && mva) {
		if (!list_empty(head)) {
			list_for_each_entry(temp, head, list) {
				DISPMSG("find buffer: temp=0x%p, mva=0x%08x\n", temp, temp->mva);
				if (mva == temp->mva) {
					buf = temp;
					break;
				}
		...
		DISPMSG("find buffer by mva, head=0x%p, buf=0x%p\n", head, buf);
	}
...
static struct task_struct *primary_path_decouple_worker_task;
...
int _disp_primary_path_set_vfp(int enter)
{
	...
	if (primary_display_is_video_mode()) {
		...
		if (enter) {
			dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle_vfp,
				DDP_DSI_VFP_LP, &(lcm_param->dsi.vertical_vfp_lp));
		} else {
...
void primary_display_lfr_enable(int enable)
{
	...
	DISPCHECK("lcm param lfr_enable 0x%x,lfr_mode 0x%x,lfr_type 0x%x,lfr_skip_num 0x%x\n",
		  lcm_param->dsi.lfr_enable, lcm_param->dsi.lfr_mode, lcm_param->dsi.lfr_type,
		  lcm_param->dsi.lfr_skip_num);
	lcm_param->dsi.lfr_enable = enable;

	DSI_Set_LFR(DISP_MODULE_DSI0, NULL, lcm_param->dsi.lfr_mode,
			lcm_param->dsi.lfr_type,
		    lcm_param->dsi.lfr_enable, lcm_param->dsi.lfr_skip_num);
...
int primary_display_save_power_for_idle(int enter, unsigned int need_primary_lock)
{
	...
	/* no need idle lock ,cause primary lock
	will be used inside switch_mode */
		...
		if (primary_display_is_video_mode() == 1) {
			...
			if (enter) {
				...
				if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE) {
					...
					if (gChangeRDMAThreshold == 1 || gIssueRequestThreshold == 1
					    || (gDisableIRQWhenIdle == 1 && gEnableIRQ == 1)) {
						cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &handle);
						cmdqRecWaitNoClear(handle,
								   CMDQ_EVENT_MUTEX0_STREAM_EOF);
					if (gChangeRDMAThreshold == 1) {
						gRDMAUltraSetting =
							gRDMAUltraSetting_Decouple;
						DISP_REG_SET(handle,
				...
				if (gChangeRDMAThreshold == 1 || gIssueRequestThreshold == 1
					...
					if (gDisableIRQWhenIdle == 1) {
						...
						reg_value =
						    DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTEN);
						DISP_REG_SET(handle, DISP_REG_CONFIG_MUTEX_INTEN,
							     reg_value | (1 << 0) | (1 <<
										     DISP_MUTEX_TOTAL));
...
/* used by aal/pwm to exit idle before config
register(for "screen idle top clock off" feature) */
...
static void _dump_internal_buffer_into(display_primary_path_context *ctx,
		char *dump_reason)
{
	int n = 0;
	int len = 0;
...
int32_t decouple_rdma_worker_callback(unsigned long data)
{
	...
	if (!reading_buf) {
		DISPERR("fatal error, we can't find related buffer info with callback data:0x%08lx\n", data);
		mutex_unlock(&(pgc->dc_lock));
		return 0;
	}
...
static int _disp_primary_path_decouple_worker_thread(void *data)
{
	...
	while (1) {
		/* shuold wait FRAME_START here, but MUTEX1's sof
		always issued with MUTEX0's sof, root cause still unkonwn. so use FRAME_COMPLETE instead */
...
*** 5.mutex enable:        N         N    idle->Y     Y        ***/
	...
	/* this is not a perfect design, we can't decide path trigger(ovl/rdma/dsi..)
	seperately with mutex enable */
	/* but it's lucky because path trigger and mutex enable is the same w/o cmdq,
	and it's correct w/ CMDQ(Y+N). */
	if (primary_display_cmdq_enabled()) {
		if (primary_display_is_video_mode())
			return 0;
		else
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		/* wait and clear stream_done, HW will assert mutex enable automatically
		in frame done reset. */
		...
		if (gEnableSWTrigger == 1) {
			DISP_REG_SET(pgc->cmdq_handle_trigger,
		...
		/* SODI is disabled in config thread, so mutex enable/dsi start
		/CPU wait TE will not be blocked by SODI */
...
static void _cmdq_start_trigger_loop(void)
{
	...
	if (gDumpConfigCMD == 1) {
		DISPMSG("ESD dump before trigger loop\n");
...
static void _cmdq_clear_config_handle_dirty(cmdqRecHandle cmdq_handle)
{
	if (!primary_display_is_video_mode()) {
		dprec_logger_start(DPREC_LOGGER_PRIMARY_CMDQ_SET_DIRTY, 0, 1);
		/* only command mode need to set dirty */
		cmdqRecClearEventToken(pgc->cmdq_handle_config, CMDQ_SYNC_TOKEN_CONFIG_DIRTY);
	}
}
...
static void directlink_path_add_memory(WDMA_CONFIG_STRUCT *p_wdma)
{
    ...
    _cmdq_clear_config_handle_dirty(cmdq_handle);
...
static int _DL_switch_to_DC_fast(unsigned int lock)
{
	...
	if (lock)
		dpmgr_modify_path(pgc->dpmgr_handle, DDP_SCENARIO_PRIMARY_RDMA0_DISP,
	...
	/* 7.reset  cmdq */
	_cmdq_reset_config_handle();
    _cmdq_clear_config_handle_dirty(pgc->cmdq_handle_config);
	_cmdq_insert_wait_frame_done_token();
		...
		for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
			...
			if (buf) {
				...
				DISPMSG(
				    ...
				    "buf->handle:0x%p,dc_vaddr = 0x%lx\n",
				     buf, (unsigned long)(&buf->list), buf->mva, buf->size,
...
int _trigger_display_interface(int blocking, void *callback, unsigned int userdata)
{
	...
	if (_need_lfr_check() == 1)
		dpmgr_path_build_cmdq(pgc->dpmgr_handle, pgc->cmdq_handle_config,
				      CMDQ_DSI_LFR_MODE);
...
int _esd_check_config_handle_vdo(void)
{
	...
	DISP_REG_SET_PA(pgc->cmdq_handle_config_esd, DISP_REG_CMDQ_TOKEN_ID,
			DISP_MUTEX0_STREAM_EOF_ID);
	...
	if (gpio_mode == GPIO_DSI_TE_PIN_M_DSI_TE)
		#endif
		{
			/* if(_need_register_eint()) */
			{
				...
				/* 2.register eint */
				node =
				    of_find_compatible_node(NULL, NULL, "mediatek, DSI_TE_1-eint");
				if (node) {
					DISPMSG("node 0x%x\n", node);
		...
		if (ret == 1) {
			...
			while (i--) {
				...
				if (ret == 0) {
					...
					if (i == 0) {
						DISPCHECK(
							"[ESD]after esd recovery %d times,"
							"esd check still fail, disable esd check\n",
						     esd_try_cnt);
...
{
	static unsigned int isFirstError;
	...
	if (primary_display_is_video_mode() == 1 && primary_display_is_decouple_mode() == 0
		...
		if (((ovl_status[0] & 0x1) != OVL_STATUS_IDLE) ||
			...
			MMProfileLogEx(ddp_mmp_get_events()->ddp_abnormal_irq, MMProfileFlagPulse,
				       ovl_status[0] | (ovl_status[1] << 16), DISP_MODULE_UNKNOWN);
			/* dump cmdq cmd here */
			ret = -1;
		}
	}
	...
	while (1) {
		if (unlikely(islcmconnected == 0 && primary_display_is_video_mode() == 0)) {
			...
			wait_event_interruptible(primary_display_present_fence_wq,
						 atomic_read
						 (&primary_display_present_fence_update_event));
		...
		/* release present fence in vsync callback */
		{
			...
			if (pgc->session_id == 0) {
				DISPMSG("_get_sync_info fail in present_fence_release thread\n");
				continue;
			}

			disp_sync_info * layer_info =
			    _get_sync_info(pgc->session_id, disp_sync_get_present_timeline_id());
			if (layer_info == NULL) {
...
int primary_display_capture_framebuffer_wdma(void *data)
{
	...
	ret =
	    m4u_alloc_mva(m4uClient, M4U_PORT_DISP_WDMA0, va[0], NULL, buffer_size,
			  M4U_PROT_READ | M4U_PROT_WRITE, 0, &mva[0]);
	if (ret != 0) {
	...
	ret =
	    m4u_alloc_mva(m4uClient, M4U_PORT_DISP_WDMA0, va[1], NULL, buffer_size,
			  M4U_PROT_READ | M4U_PROT_WRITE, 0, &mva[1]);
	if (ret != 0) {
	...
	if (primary_display_cmdq_enabled()) {
		...
		while (primary_dump_wdma) {
			...
			ret =
			    m4u_cache_sync(m4uClient, M4U_PORT_DISP_WDMA0, va[buf_idx], buffer_size,
					   mva[buf_idx], M4U_CACHE_FLUSH_BY_RANGE);
			_primary_path_lock(__func__);
			pconfig = dpmgr_path_get_last_config(pgc->dpmgr_handle);
			pconfig->wdma_dirty = 1;
...
int primary_display_release_fence_fake(void)
{
	...
	for (i = 0; i < PRIMARY_DISPLAY_SESSION_LAYER_COUNT; i++) {
		if (i == primary_display_get_option("ASSERT_LAYER") && is_DAL_Enabled()) {
			...
			disp_sync_get_cached_layer_info(session_id, i, &layer_en, &addr,
							&fence_idx);
			if (fence_idx < 0) {
				if (fence_idx == -1) {
					DISPPR_ERROR
					    ...
					    i, 0, fence_idx);
				} else if (fence_idx == -2) {/* do nothing*/
...
int primary_display_mem_out_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (_should_flush_cmdq_config_handle())
		...
		cmdqRecFlushAsyncCallback(pgc->cmdq_handle_ovl1to2_config,
					  primary_display_ovl2mem_callback, userdata);
...
int primary_display_config_output(disp_mem_output_config *output)
{
	...
	if ((pgc->session_id > 0) && _is_decouple_mode(pgc->session_mode))
		update_frm_seq_info(output->addr, 0,
			     ...
			     disp_path_handle disp_handle, cmdqRecHandle cmdq_handle)
{
	...
	unsigned int blend_layer_num_draw_on_layer_id = 0;
	unsigned int blend_layer_num = 0;
#endif
 /*=== create new data_config for ovl input ===*/
	...
	if (gAddFrame == 1) {
		for (i = 0; i < session_input->config_layer_num; i++) {
			...
			if (input_cfg->layer_enable == 1) {
				blend_layer_num++;
				if (input_cfg->tgt_offset_x <= 16 &&
				    input_cfg->tgt_offset_y >=
				    primary_display_get_height() - input_cfg->tgt_height
				    && input_cfg->tgt_width >= DISP_DIGIT_W
				    && input_cfg->tgt_height >= DISP_DIGIT_H
				    && input_cfg->layer_id > blend_layer_num_draw_on_layer_id) {
					blend_layer_num_draw_on_layer_id = input_cfg->layer_id;	/* top layer */
				}
			}
		}
	}
...
#ifdef DISP_ENABLE_LAYER_FRAME
		/* add an colorful frame on the buffer */
		if (gAddFrame == 1 && ovl_cfg->layer_en == 1) {
			unsigned int size = ovl_cfg->src_pitch * ovl_cfg->dst_h;
			unsigned long va, va_end;
			unsigned int mapped_size;
			int ret;
			unsigned int j = 0, w = 0;
			unsigned int bpp = ovl_cfg->src_pitch / ovl_cfg->dst_w;

			ret =
			    m4u_mva_map_kernel(ovl_cfg->addr, size, (unsigned int *)&va,
					       &mapped_size);
			if (ret == 0) {
				/* caculate frame width */
				for (j = 0; j < i; j++) {
					if (ovl_cfg->dst_w == session_input->config[j].tgt_width &&
					    ovl_cfg->dst_x == session_input->config[j].tgt_offset_x
					    && frame_width > 4) {
						frame_width -= 4;
						break;
					}
				...
				/* draw frame */
				if (mapped_size >= ovl_cfg->src_pitch * frame_width)
					mapped_size = ovl_cfg->src_pitch * frame_width;

				for (j = 0; j < mapped_size / 4; j++)
					*(unsigned int *)(va + j * 4) = disp_color[ovl_cfg->layer];

				va_end = va + ovl_cfg->src_pitch * ovl_cfg->dst_h;
				for (j = 0; j < mapped_size / 4; j++)
					*(unsigned int *)(va_end - (j + 1) * 4) =
					    disp_color[ovl_cfg->layer];

				for (j = 0; j < ovl_cfg->dst_h; j++) {
					for (w = 0; w < frame_width; w++) {
						*(unsigned int *)(va + w * 4 +
								  j * ovl_cfg->src_pitch) =
						    disp_color[ovl_cfg->layer];
						*(unsigned int *)(va - (w + 1) * 4 +
								  (j + 1) * ovl_cfg->src_pitch) =
						    disp_color[ovl_cfg->layer];
					}
				}

				/* draw digits */
				if (ovl_cfg->dst_w > DISP_DIGIT_W && ovl_cfg->dst_h > DISP_DIGIT_H) {
					unsigned int x_offset = 0;

					for (j = 0; j <= i; j++) {

						/* if j<i, means there are some numbers at same position,
						need move new number serveral rows behind */
					if (session_input->config[j].tgt_offset_x == ovl_cfg->dst_x
							&& session_input->config[j].tgt_offset_y ==
						    ovl_cfg->dst_y
						    && session_input->config[j].layer_enable == 1
						    && session_input->config[j].buffer_source !=
						    DISP_BUFFER_ALPHA) {
							/* re-draw lower layer number on top layer */
							disp_draw_num(va,	/* dst buf va */
								      x_offset,	/* x offset (pixel) */
								      0,	/* y offset */
								      ovl_cfg->src_pitch,	/* dst pitch */
								      ovl_cfg->fmt,	/* dst buf format */
								      j,	/* digit number */
								      0,	/* transparent */
								      disp_color[j]);	/* background oqupa or not */

							x_offset += DISP_DIGIT_W;
						}
					}
				...
				/* draw total number */
				if (i == blend_layer_num_draw_on_layer_id) {
					/* enabled layer total number */
					DISPMSG("draw total number! i=%d, blend_layer_num=%d\n", i,
						blend_layer_num);
					disp_draw_num(va,	/* dst buf va */
						      8,	/* x offset (pixel) */
						      ovl_cfg->dst_h - DISP_DIGIT_H - 8,	/* y offset */
						      ovl_cfg->src_pitch,	/* dst pitch */
						      ovl_cfg->fmt,	/* dst buf format */
						      blend_layer_num,	/* digit number */
						      0, 0xff000000);	/* background oqupa or not */
				}
			}
...
int primary_display_user_cmd(unsigned int cmd, unsigned long arg)
{
	...
	MMProfileLogEx(ddp_mmp_get_events()->primary_display_cmd, MMProfileFlagEnd, handle,
		       cmdqsize);
	return ret;
}
...
int _set_backlight_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		...
		cmdqRecReset(cmdq_handle_backlight);
        _cmdq_clear_config_handle_dirty(cmdq_handle_backlight);
...
int primary_display_setbacklight(unsigned int level)
{
...
#if 0				/* check writed success? for test after CABC */
	{
		...
		DISPMSG("[CABC check result 0x51 = 0x%x,0x%x]\n", buffer[0], buffer[1]);
		dpmgr_path_start(pgc->dpmgr_handle, CMDQ_DISABLE);
		if (primary_display_is_video_mode()) {
			/* for video mode, we need to force trigger here */
			/* for cmd mode, just set DPREC_EVENT_CMDQ_SET_EVENT_ALLOW when trigger loop start */
...
#if 0
/* do not have to call enable_cascade here, dpmgr_path_add_memout()
      will add ovl1 if necessary */
				  ...
				  unsigned long *mva)
{
	...
	*va = (unsigned long)ioremap_nocache(pa_start, pa_end - pa_start + 1);
	DISPMSG("disphal_allocate_fb, pa=%p, va=0x%lx\n", &pa_start, *va);
...
void primary_display_reset_ovl_by_cmdq(unsigned int force)
{
	...
	if (force == 0) {
		...
		/* reset ovl0 */
		DISP_REG_SET_FIELD(pgc->cmdq_handle_config, RST_FLD_FORCE_RST, DISP_REG_OVL_RST,
				   0x1);
		DISP_REG_SET_FIELD(pgc->cmdq_handle_config, RST_FLD_FORCE_RST, DISP_REG_OVL_RST,
				   0x0);
		cmdqRecPoll(pgc->cmdq_handle_config, disp_addr_convert(DISP_REG_OVL_STA), 0, 0x1);
...
int primary_display_te_test(void)
{
	...
	if (ret >= 0)
		DISPMSG("[display_test_result]==>Force On TE Open!\n");
	else
		DISPMSG("[display_test_result]==>Force On TE Closed!\n");
...
int primary_display_rdma_res_test(void)
{
	...
	if (ret < 0)
		DISPMSG("[display_test_result]==>RDMA RESOLUTION SETTING !\n");
	else
		DISPMSG("[display_test_result]==>RDMA RESOLUTION SETTING ONCE!\n");
	return ret;
}
