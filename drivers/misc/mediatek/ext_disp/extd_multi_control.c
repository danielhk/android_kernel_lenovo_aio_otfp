...
#include "./mt6752/extd_platform.h"
#include "extd_log.h"
#include "mtk_ovl.h"
...
static int extd_create_path(EXT_DISP_PATH_MODE mode, unsigned int session)
{
	int ret = 0;
...
void external_display_control_init(void)
{
	...
	int i = 0;
	memset(&path_info, 0, sizeof(SWITCH_MODE_INFO_STRUCT));
	path_info.switching = DEV_MAX_NUM;
...
{
	int ret = 0;
	EXTD_OVL_REQ_STATUS ovl_status = EXTD_OVL_NO_REQ;
