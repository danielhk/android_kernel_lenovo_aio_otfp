...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		params->dsi.vertical_sync_active				= 10;// 3    2
        ...
        params->dsi.PLL_CLOCK = 200; //390 for 120fps//320;//dsi clock customization: should config clock value directly    	
...
static unsigned int lcm_compare_id(void)
{
    ...
    #else
		pr_debug("%s, kernel otm1282a horse debug: otm1282a id = 0x%08x\n", __func__, id);
    #endif
...
// ---------------------------------------------------------------------------
LCM_DRIVER otm1282a_hd720_dsi_vdo_60hz_lcm_drv = 
{
