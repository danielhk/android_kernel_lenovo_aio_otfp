...
#define PFX "OV13850SUNNY_R2A_OTP"
...
int ov13850sunnyr2a_update_eeprom()
{
...
bool ov13850sunny_otp_check_mid(void)
{
  u32 mid = 0x00;
  u32 OTP_SENSOR_MID=0;
  TRACE("ov13850sunny_otp_check_mid start\n");    
  OTP_SENSOR_MID = (u32)ov13850r2a_read_cmos_sensor(OTP_READ_ID,0x0401);
  TRACE("%s SUNNY MID=0x%02x \r\n",__func__,OTP_SENSOR_MID);

  if (OTP_SENSOR_MID==0x01) //sunnny =0x01,,OFLIM=0x07
  {
    return TRUE;
  }
  else
...
/*lenovo.sw START wangsx3 add for AF OTP */
kal_uint8 ov13850_cam_cal_read_data(kal_uint32 offset,
