...
#include "ov5670litemipi_Sensor.h"

#ifdef CONFIG_COMPAT
#include <linux/compat.h>
#endif
#define PFX "ov5670lite_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670LITEMIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
bool ov5670lite_otp_check_mid(void)
{
  u32 mid = 0x00;
  ...
  if (OTP_SENSOR_MID==0x04)//sunnny =0x01,,OFLIM=0x07,VCM_ID :OFILM A40=0x08,OFILM A50=0x04,
  {
    return TRUE;
  }
  else
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
    ...
    LOG_INF("ov5670lite get_imgsensor_id start\n");
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C))+1;
			if (*sensor_id == imgsensor_info.sensor_id) {				
                ...
                check_mid=ov5670lite_otp_check_mid();
                if (!check_mid)
                {
