...
#include "hi551qtechv2_mipi_Sensor.h"
...
static int HI551OTP_Check(void) {
	...
	//check OTP cal version 
	for (j =0; j<3; j++) {
		if(((hi551Otp.module_i.module_flag >> (3-j)*2) & 0x3) == 1) { //group is valid
			offset = 1;
			addr = OTPV2_MODULE_GROUP_ONE;
			if (j  == 1) {
				addr = OTPV2_MODULE_GROUP_TWO;
			}else if( j == 2) {
				addr = OTPV2_MODULE_GROUP_THREE;
			}
    	...
    	LOG_INF("otp_version = %x\n ",hi551Otp.otp_version);
     	//OTP version 2
	if( hi551Otp.otp_version == 2 )   {
		//read module info
		if (HI551OTP_ReadAll(addr, addr + MODULE_V2_LENGTH -1, hi551Otp.module_i.data.module_data) < 0)
			goto otp_error;
		//HI551OTP_DUMP(otp_buffer,OTPBUFFER_SIZE);

		//module group
		checksum = 0; 
		for(i=0 ; i < MODULE_V2_LENGTH-1; i++) {
			value =  hi551Otp.module_i.data.module_data[i];
			checksum += value;
		}
		hi551Otp.module_i.checksum = hi551Otp.module_i.data.module_data[MODULE_V2_LENGTH -1];
		...
		//READ AWB OTP INFO
		//AWB table 
		hi551Otp.awb_i.awb_flag = HI551OTPSensorReg(OTPV2_AWB_FLAG_ADDR);
		...
		for (j =0; j<3; j++) {
			if(((hi551Otp.awb_i.awb_flag >> (3-j)*2) & 0x3) == 1) { //group is valid
				addr = OTPV2_AWB_GROUP_ONE;          
				if (j ==1) {
					addr = OTPV2_AWB_GROUP_TWO;
				}else if (j==2) {
					addr = OTPV2_AWB_GROUP_THREE;
				}
		...
		//READ LSC INFO
		hi551Otp.lsc_i.lsc_flag = HI551OTPSensorReg(OTPV2_LSC_FLAG_ADDR);
		LOG_INF("LSC info flag= %x ",hi551Otp.lsc_i.lsc_flag);
		if(!(hi551Otp.lsc_i.lsc_flag & (1 <<2 |1 << 4|1<<6)) )  {
			LOG_INF("no valid lsc info is found ");
			goto otp_error;
		}
		for (j =0; j<3; j++) {
			if(((hi551Otp.lsc_i.lsc_flag >> (3-j)*2) & 0x3) == 1) { //group is valid
				addr = OTPV2_LSC_GROUP_ONE;          
				if (j ==1) {
					addr = OTPV2_LSC_GROUP_TWO;
				}else if (j==2) {
					addr = OTPV2_LSC_GROUP_THREE;
				}
				hi551Otp.lsc_i.group = j;
				break;
			}
		}
		if (HI551OTP_ReadAll(addr, addr + LSC_LENGTH - 1, hi551Otp.lsc_i.data) < 0)
			goto otp_error;
		checksum = 0;
		for(i=0 ; i<LSC_LENGTH -1; i++) {
			value =   hi551Otp.lsc_i.data[i];
			checksum += value;
		}
		hi551Otp.lsc_i.checksum = hi551Otp.lsc_i.data[LSC_LENGTH -1];
		LOG_INF("LSC  group %d checksum =%x, %x\n",hi551Otp.lsc_i.group,checksum,(kal_uint32)hi551Otp.lsc_i.checksum);
		checksum = checksum%0xFF +1;
		if(hi551Otp.lsc_i.checksum != checksum) {
			LOG_INF("LSC  group %d checksum error\n",hi551Otp.lsc_i.group);
			goto otp_error;
		}
		hi551Otp.lsc_i.lsc_info_valid = 1;
		LOG_INF("LSC  group %d checksum valid\n",hi551Otp.lsc_i.group);

		if (hi551Otp.module_i.module_info_valid ==1 && hi551Otp.awb_i.awb_info_valid ==1 && hi551Otp.lsc_i.lsc_info_valid == 1) 
			return 0;
       } // end of  if cal_ver ==1 
...
void HI551QTECHV2_OTP_ApplyWB(bool apply)
{
...
bool isHI551QTECHV2haveOTP(void) {
	return (hi551Otp.otp_valid_flag == 1);
}

int HI551QTECHV2OTP_OTP_Check(void)  {
	int i,ret;
...
int HI551QTECHV2_OTP_Ver_Check(void)
{
...
kal_uint32 hi551_cam_cal_read_data(kal_uint32 offset,
        kal_uint32 lenth, kal_uint8 *pBuf)
{
	//return accual read lenth
        kal_uint32 i;

      	LOG_INF("Enter %s,offset=0x%x,len=%d\n",
                __func__,offset,lenth);
	...
	if (lenth < (LSC_LENGTH-1)) { //452 bytes
		up(&otp_sem);
		return 0;
	}

	memcpy(pBuf, hi551Otp.lsc_i.data, (LSC_LENGTH-1));

	for(i=0;i<452;i++){
		printk("yao,hi551 otp debug: lsc[%d]=0x%x,buf[%d]=0x%x\n",
			i,hi551Otp.lsc_i.data[i],i,pBuf[i]);
