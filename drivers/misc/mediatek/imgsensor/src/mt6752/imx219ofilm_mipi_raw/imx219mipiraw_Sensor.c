...
#define	IMX219_OTP_DBG(format,args...) xlog_printk(ANDROID_LOG_ERROR,"IMX219_OTP",format,##args)
#else
...
static kal_uint8 imx219_init_otp()
{
	...
	down_interruptible(&imx219_otp_sem);//Yao,check this 
...
static void imx219_auto_load_lsc()
{
...
static void imx219_try_init_otp()
{
	int i = 0;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_INF("Read sensor id fail, i2c_id=0x%x,id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
				LOG_INF("Warning: current_fps fps is not support, so use cap1's setting: fps!\n");
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_CROP_INFO:
            //LOG_INF("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%lu\n", *feature_data);
