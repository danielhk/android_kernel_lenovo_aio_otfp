...
kal_uint32 bq24196_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq24196_read_interface] Reg[%x]=0x%x\n", RegNum, bq24196_reg);

    bq24196_reg &= (MASK << SHIFT);
    *val = (bq24196_reg >> SHIFT);
	battery_log(BAT_LOG_CRTI, "[bq24196_read_interface] Val=0x%x\n", *val);
...
kal_uint32 bq24196_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = bq24196_read_byte(RegNum, &bq24196_reg);
	battery_log(BAT_LOG_CRTI, "[bq24196_config_interface] Reg[%x]=0x%x\n", RegNum, bq24196_reg);

    bq24196_reg &= ~(MASK << SHIFT);
    bq24196_reg |= (val << SHIFT);

    ret = bq24196_write_byte(RegNum, bq24196_reg);
	battery_log(BAT_LOG_CRTI, "[bq24196_config_interface] Write Reg[%x]=0x%x\n", RegNum, bq24196_reg);

    // Check
    //bq24196_read_byte(RegNum, &bq24196_reg);
	/*battery_log(BAT_LOG_CRTI, "[bq24196_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24196_reg);*/
...
{
    kal_uint8 i=0;
	battery_log(BAT_LOG_CRTI, "[bq24196] ");
    for (i=0;i<bq24196_REG_NUM;i++)
    {
        bq24196_read_byte(i, &bq24196_reg[i]);
		battery_log(BAT_LOG_CRTI, "[0x%x]=0x%x ", i, bq24196_reg[i]);
    }
	battery_log(BAT_LOG_CRTI, "\n");
}
...
static ssize_t store_bq24196_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        ...
        if(size > 3)
        {
			...
			battery_log(BAT_LOG_CRTI, "[store_bq24196_access] write bq24196 reg 0x%x with value 0x%x !\n", reg_address, reg_value);
        ...
        else
        {
			...
			battery_log(BAT_LOG_CRTI, "[store_bq24196_access] read bq24196 reg 0x%x with value 0x%x !\n", reg_address, g_reg_value_bq24196);
			battery_log(BAT_LOG_CRTI, "[store_bq24196_access] Please use \"cat bq24196_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int __init bq24196_subsys_init(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[bq24196_init] init start. ch=%d\n", BQ24196_BUSNUM);
