...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
	...
	if (path) {
		...
		fw_checksum = 0;
		for (i = 0;
		     i < FW_SECTION_LENGTH * 4 + FW_DSP_ISP_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH;
		     i += 2) {
			u16 temp;
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client, 0xC000, &gtp_default_FW_fl[FW_HEAD_LENGTH], FW_DOWNLOAD_LENGTH);	/* write the first bank */
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
	...
	ret = i2c_write_bytes(client, 0xC000, &gtp_default_FW_fl[FW_HEAD_LENGTH + 2 * FW_DOWNLOAD_LENGTH], FW_DSP_LENGTH);	/* write the second bank */
	if (ret == -1) {
