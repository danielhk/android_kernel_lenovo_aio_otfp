...
static u8 sensor_id;	/* add  by yangjuwei */
...
static int gt91xx_config_read_proc(char *page, char **start, off_t off, int count, int *eof,
				   void *data)
{
	char *ptr = page;
...
static int gt91xx_config_write_proc(struct file *file, const char *buffer, unsigned long count,
				    void *data)
{
	s32 ret = 0;
...
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	if (cfg_len < GTP_CONFIG_MIN_LENGTH) {
		GTP_ERROR
		    ("CTP_CONFIG_GROUP%d is INVALID CONFIG GROUP! NO Config Sent! You need to check you header file CFG_GROUP section!",
		     sensor_id + 1);
		pnl_init_error = 1;
		return -1;
	}
						   ...
						   struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf,
		       "TP IC :%s,TP module :%s,ic firmware vesion: 0x%x,file firmware version: 0x%x,",
		       ic_g, module_name_g, ic_fw_version_g, file_fw_version_g);
}
...
static DEVICE_ATTR(upgrade_node_self_apk, 0664, firmware_node_upgrade_self_apk_show,
		   firmware_node_upgrade_self_apk_store);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!int_type)		/* EINTF_TRIGGER */
	{
		...
		mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, EINTF_TRIGGER_FALLING,
				     tpd_eint_interrupt_handler, 1);
	}
...
/* add end */
	...
	len =
	    sprintf(tp_info,
		    "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,ic firmware version:0x%x,file firmware version: 0x%x,",
		    ic_g, module_name_g, client->addr, ic_fw_version_g, file_fw_version_g);
...
{
	GTP_DEBUG("gt9xx_2 tpd_local_init\n");
#if GTP_ESD_PROTECT
	...
	GTP_DEBUG("gt9xx_2 i2c_add_driver ok\n");
