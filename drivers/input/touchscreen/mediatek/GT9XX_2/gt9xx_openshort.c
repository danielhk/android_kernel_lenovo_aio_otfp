...
u16 screen_key_limit_arry[6][4] = { {5160, 970, 1620, 700}
, {}
, {}
, {4680, 960, 1560, 650}
, {}
	...
	while (left > 0) {
		...
		for (i = 0; i < opr_len; ++i) {
			if (opr_buf[i + 2] != dsp_short[addr - GTP_REG_DSP_SHORT + i]) {
				...
				gtp_write_register(client, addr + i,
...
s32 gtp_compute_rslt(struct i2c_client *client)
{
	...
	short_sum =
	    (struct gt9xx_short_info *)kmalloc(sizeof(struct gt9xx_short_info) * _SHORT_INFO_MAX,
					       GFP_KERNEL);
	...
	for (i = 0; i < tx_short_num; ++i) {
		...
		for (j = i + 1; j < MAX_DRIVER_NUM; ++j) {
			short_code =
			    (result_buf[2 + 3 + j * 2] << 8) + result_buf[2 + 3 + j * 2 + 1];
		...
		for (j = 0; j < MAX_SENSOR_NUM; ++j) {
			short_code =
			    (result_buf[2 + 3 + 84 + j * 2] << 8) + result_buf[2 + 3 + 84 + j * 2 +
									       1];
	...
	if (node_idx == 0) {
		...
		for (i = 0, j = 0; i < node_idx; ++i) {
			...
			if ((short_sum[i].master_is_driver)) {
				master =
				    gt9_get_short_tp_chnl(short_sum[i].master - GT9_DRV_HEAD, 1);
			} else {
	...
	for (i = 0; i < len; i += 2) {
...
#ifdef GTP_VDD
			r = 40 * 9 * 1024 * (100 * GTP_VDD - 900) / ((short_code & (~0x8000)) * 7) -
			    40 * 1000;
		...
		if (short_res == 0xFFFF) {
			...
			if (short_res < (gt900_gnd_resistor_threshold * amplifier)) {
				...
				{
					short_chnl =
					    gt9_get_short_tp_chnl(ChannelPackage_TX[i / 2], 1);
					...
					short_chnl =
					    gt9_get_short_tp_chnl((i / 2) - MAX_DRIVER_NUM, 0);
...
load_dsp_again:
	...
	GTP_DEBUG("\n IC Sensor:");
	for (i = 0; i < MAX_SENSOR_NUM; ++i) {
		GTP_DEBUG(" %2d", cfg_sen_order[i]);
	}
	GTP_DEBUG("\N TP Sensor:");
	for (i = 0; i < MAX_SENSOR_NUM; ++i) {
		GTP_DEBUG(" %2d", i);
	}
	GTP_DEBUG("\n");
...
s32 gtp_read_rawdata(struct i2c_client *client, u16 *data)
{
	...
	read_rawbuf =
	    (u8 *) kmalloc(sizeof(u8) * (gt9xx_drv_num * gt9xx_sen_num * 2 + GTP_ADDR_LENGTH),
			   GFP_KERNEL);
...
static s32 gtp_raw_test_init(void)
{
	...
	touchpad_sum =
	    (struct gt9xx_open_info *)kmalloc(sizeof(struct gt9xx_open_info) *
					      (4 * _BEYOND_INFO_MAX + 1), GFP_ATOMIC);
	...
	for (i = 0; i < gt9xx_sc_pxl_cnt; i++) {
		if (raw_buf[i] < min_limit_value) {
			...
			if (new_flag)	/* new one */
			{
				...
				GTP_DEBUG("[%d, %d]rawdata: %d, raw min limit: %d", driver, sensor,
					  raw_buf[i], min_limit_value);
			} else {
				continue;
			}
		}
	...
	for (i = 0; i < gt9xx_sc_pxl_cnt; i++) {
		if (raw_buf[i] > max_limit_value) {
			...
			if (new_flag)	/* new one */
			{
				...
				touchpad_sum[j].raw_val = raw_buf[i];
				touchpad_sum[j].times = 1;
				GTP_DEBUG("[%d, %d]rawdata: %d, raw max limit: %d", driver, sensor,
					  raw_buf[i], max_limit_value);
			} else {
				continue;
			}
		}
	...
	for (i = gt9xx_sc_pxl_cnt; i < gt9xx_pixel_cnt; ++i) {
		...
		if (raw_buf[i] > max_limit_key) {
			...
			if (new_flag)	/* new one */
			{
				...
				GTP_DEBUG("[%d, %d]key rawdata: %d, key max limit: %d", driver,
					  sensor, raw_buf[i], max_limit_key);
			} else {
				continue;
			}
		}
	...
	for (i = gt9xx_sc_pxl_cnt; i < gt9xx_pixel_cnt; ++i) {
		...
		if (raw_buf[i] < min_limit_key) {
			...
			if (new_flag)	/* new one */
			{
				...
				GTP_DEBUG("[%d, %d]key rawdata: %d, key min limit: %d", driver,
					  sensor, raw_buf[i], min_limit_key);
...
static void gtp_uniformity_test(u16 *raw_buf)
{
	...
	if (uniformity < uniformity_lmt) {
		...
		GTP_INFO("min_val: %d, max_val: %d, uniformity: %d%%, times: %d", min_val, max_val,
...
static s32 gtp_get_test_result(void)
{
...
#if GTP_DEBUG_ON
	for (i = 0; i < 4 * _BEYOND_INFO_MAX; ++i) {
		GTP_DEBUG("(%2d, %2d)[%2d] ", touchpad_sum[i].driver, touchpad_sum[i].sensor,
	...
	if (have_key) {
		if (beyond_key_max > _MIN_ERROR_NUM) {
			...
			for (i = 2 * _BEYOND_INFO_MAX; i < (3 * _BEYOND_INFO_MAX); ++i) {
				...
				SET_INFO_LINE_INFO("  Drv: %d, Sen: %d RawVal: %d",
						   touchpad_sum[i].driver, touchpad_sum[i].sensor,
						   touchpad_sum[i].raw_val);
			}
		}
	...
	if (beyond_uniformity > _MIN_ERROR_NUM) {
		...
		SET_INFO_LINE_INFO("  Uniformity Limit: %d%%, Tp Uniformity: %d%%", uniformity_lmt,
				   touchpad_sum[4 * _BEYOND_INFO_MAX].raw_val / touchpad_sum[4 *
											     _BEYOND_INFO_MAX].
				   times);
	}
	...
	if (have_key) {
		if (result & _BEYOND_KEY_MAX_LMT) {
			SET_INFO_LINE_INFO("  Beyond KeyVal Max Limit [Key Max Limit: %d]",
					   max_limit_key);
		}
		if (result & _BEYOND_KEY_MIN_LMT) {
			SET_INFO_LINE_INFO("  Beyond KeyVal Min Limit [Key Min Limit: %d]",
					   min_limit_key);
		}
	}

	if (result & _BEYOND_UNIFORMITY_LMT) {
		SET_INFO_LINE_INFO("  Beyond Uniformity Limit [Uniformity Limit: %d%%]",
				   uniformity_lmt);
	}
	return result;
}
...
static ssize_t gtp_sysfs_shorttest_show(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	u8 index;
	ssize_t len;
...
static ssize_t gtp_sysfs_shorttest_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t gtp_sysfs_opentest_store(struct device *dev, struct device_attribute *attr,
					const char *buf, size_t count)
{
	return -EPERM;
}
